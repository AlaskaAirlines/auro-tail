...
import { badgeLogos, LINKS_SIZES, BADGES_SIZES } from './constants';
import { resolveBorderProps, normalizeBorderWidth } from './utils';
 ...
 * @attr {String} badge - Sets the badge type to display (e.g., 'oneworld'). Only shown when not in a group.
...
export class AuroTail extends LitElement {
  ...
  constructor() {
    ...
    /**
     * Private property that determines the carrier variant based on tail code.
     ...
     * event is dispatched. Not exposed as an attribute.
     * @private
  ...
  /**
   * Gets the carrier variant based on the current tail code, mapping tail codes to carrier types for styling purposes.
   * @readonly
  ...
  /**
   * Returns badge config when eligible (not grouped and size supports badges).
   * @readonly
   * @returns {BadgeConfig | undefined}
   */
  get badgeConfig() {
    return this.badge && !this.isInGroup && BADGES_SIZES.includes(this.size) 
      ? badgeLogos[/** @type {keyof typeof badgeLogos} */ (this.badge)] 
      : undefined;
  }
    ...
    if (this.shouldShowLink) {
      return html`
        <div class="tail">
          ...
          >
            ${tailContent}
            <slot name="display" class="label ${this.labelTypeClass}">Airline</slot>
          </auro-hyperlink>
        </div>
      `;
    }
